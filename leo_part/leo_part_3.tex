...
\begin{lstlisting}[language=Python, caption={Exponential fit function and curve fitting procedure.}, frame=single, framesep=8pt]
def exp_fun(x, a, b):
    return a * np.exp(b * x)
...
As shown in Fig.~\ref{fig:plot1}, the exponential fit matches the numerical data reasonably well. However, despite the data points appearing somewhat linear, this is primarily due to the limited number of sampled points, which is insufficient to fully resolve the true exponential behavior over a wider range.
